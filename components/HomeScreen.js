...
export default function HomeScreen({ navigation }) {
  ...
  const decreaseNumber = (number, type) => {
    type === "sets"
      ? setSetsNum(number > 1 ? number - 1 : number)
      : setRoundsNum(number > 1 ? number - 1 : number);
  };

  const setNumber = (number, type) => {
    type === "sets"
      ? setSetsNum(number != "" ? Number(number) : null)
      : setRoundsNum(number != "" ? Number(number) : null);
  };
  ...
  const setMinutes = (minutes, type, action) => {
    ...
    if (action === "done") {
      if (minutes === null) {
        ...
        if (type === "rest") {
          setRestMin(minutes >= 10 ? `${minutes}` : `0${minutes}`);
        }
        if (type === "roundsRest") {
          setRoundsRestMin(minutes >= 10 ? `${minutes}` : `0${minutes}`);
        }
      }
    }
  };
  ...
  const setSeconds = (seconds, type, action) => {
    ...
    if (action === "done") {
      if (seconds === null) {
        ...
        if (type === "rest") {
          setRestSec(seconds >= 10 ? `${seconds}` : `0${seconds}`);
        }
        if (type === "roundsRest") {
          setRoundsRestSec(seconds >= 10 ? `${seconds}` : `0${seconds}`);
        }
      } else {
        ...
        if (type === "work") {
          setWorkSec(
            remainingSec >= 10 ? `${remainingSec}` : `0${remainingSec}`
          );
          setWorkMin(
            calculatedMin >= 10 ? `${calculatedMin}` : `0${calculatedMin}`
          );
        }
        if (type === "rest") {
          setRestSec(
      ...
      <TouchableOpacity
        onPress={() =>
          navigation.navigate("Timer", {
            sets: setsNum,
            rounds: roundsNum,
            work: [workMin, workSec],
            rest: [restMin, restSec],
            roundsRest: [roundsRestMin, roundsRestSec]
          })
        }
      >
