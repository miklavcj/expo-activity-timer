...
export default function ActivityScreen({ route, navigation }) {
  ...
  // const [pauseTimer, setPausrTimer] = useState(false);
  ...
  const [roundsRestMin, setRoundsRestMin] = useState(
    route.params.roundsRest[0]
  );
  const [roundsRestSec, setRoundsRestSec] = useState(
    route.params.roundsRest[1]
  );
  ...
  return <View style={styles.container}>
	{timerScreen}
	<Button title="Pause / Start" onPress={()=> clearInterval(interval)}/>
	</View>;
}
